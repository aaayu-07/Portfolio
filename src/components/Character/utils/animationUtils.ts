...
const headBoneNames = ["spine.005", "spine.006", "spine005", "spine006"];

const setAnimations = (gltf: GLTF) => {
  let character = gltf.scene;
  let mixer = new THREE.AnimationMixer(character);

  // Create the filtered intro clip ONCE so both setup and startIntro share the
  // same clip UUID → same cached AnimationAction → LoopOnce is preserved.
  let filteredIntroClip: THREE.AnimationClip | null = null;
  ...
  if (gltf.animations) {
    ...
    if (introClip) {
      filteredIntroClip = removeAnimationTracks(introClip, headBoneNames);
      const introAction = mixer.clipAction(filteredIntroClip);
  ...
  function startIntro() {
    // Reuse the same filteredIntroClip → same cached action → LoopOnce is intact
    if (!filteredIntroClip) return;
    const introAction = mixer.clipAction(filteredIntroClip);
    introAction.clampWhenFinished = true;
...
const removeAnimationTracks = (
  clip: THREE.AnimationClip,
  excludedBoneNames: string[]
): THREE.AnimationClip => {
  const filteredTracks = clip.tracks.filter(
    (track) =>
      !excludedBoneNames.some((boneName) => track.name.includes(boneName))
  );

  return new THREE.AnimationClip(
    clip.name + "_without_head",
    clip.duration,
    filteredTracks
  );
};
